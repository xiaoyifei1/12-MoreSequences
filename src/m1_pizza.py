...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(75, 50),  # All numbers are approximate.
                rg.Point(50, 75),
                rg.Point(25, 50)]
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(generate_points_on_circle(circle, number_of_slices))):
        l = rg.Line(generate_points_on_circle(circle, number_of_slices)[k], circle.center)
        l.color = color
...
def run_test_polygon():
    ...
    # -------------------------------------------------------------------------
    title = 'POLYGON tests 3:  5 segments with thick pink lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(100, 100), 50)
    circle.outline_thickness = 1
    polygon(window, circle, 3, 'pink', 20)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(generate_points_on_circle(circle, number_of_segments)) - 1):
        l = rg.Line(generate_points_on_circle(circle, number_of_segments)[k],
                    generate_points_on_circle(circle, number_of_segments)[k + 1])
        l.color = color
        l.thickness = thickness
        l.attach_to(window)

    l = rg.Line(generate_points_on_circle(circle, number_of_segments)[
                    len(generate_points_on_circle(circle, number_of_segments)) - 1],
                generate_points_on_circle(circle, number_of_segments)[0])
    l.color = color
    l.thickness = thickness
    l.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    # -------------------------------------------------------------------------
    title = 'FANCY POLYGON test 4:  25 pink lines on orange circle, hops = 3.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 200)
    circle.fill_color = 'pink'
    fancy_polygon(window, circle, 25, 3, 'orange', 2)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(generate_points_on_circle(circle, number_of_lines))):
        l = rg.Line(generate_points_on_circle(circle, number_of_lines)[k],
                    generate_points_on_circle(circle, number_of_lines)[
                        (k + hops_to_next_point) % len(generate_points_on_circle(circle, number_of_lines))])
        l.arrow = 'last'
        l.thickness = thickness
        l.color = color
        l.attach_to(window)
